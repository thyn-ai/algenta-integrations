"""Asserts `litellm_algenta.contract`'s embedded constants agree with the real
...
always runs there and is the real enforcement mechanism against contract/package drift.
"""
...
def _find_contract_file() -> Path | None:
    # tests/ -> litellm-algenta/ -> python/ -> repo root
...
def test_execute_decision_model_facing_params_exclude_never_model_facing_fields(contract: dict) -> None:
    # This package's own addition (the contract doesn't enumerate execute_decision's *allowed*
    # params, only the ones that must never be model-facing) -- but it must never overlap with
    # the contract's banned set, in any circumstance.
    never_facing = frozenset(contract["profiles"]["execute"]["never_model_facing_fields"])
    assert not (EXECUTE_DECISION_MODEL_FACING_PARAMS & never_facing)
