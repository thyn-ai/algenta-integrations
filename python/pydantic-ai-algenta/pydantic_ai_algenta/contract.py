...
discovered at `get_tools()` time, not known ahead of time by this package.
"""
...
#: contract's `profiles.execute.never_model_facing_note`. `AlgentaToolset` enforces this at two
#: layers: it strips these keys from a tool's advertised JSON schema (`get_tools`) *and* from the
#: arguments dict actually forwarded to the wrapped MCP tool call (`call_tool`), so a model that
#: somehow still emitted one of these as an argument (e.g. by copying it from a prior message)
#: can't get it through either.
