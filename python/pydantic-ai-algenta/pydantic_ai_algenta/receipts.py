...
as its result. `AlgentaToolset.call_tool` parses the wrapped MCP tool's raw result into this
model so a caller gets a typed object rather than an untyped dict -- and so the envelope's
`approval_state` and `code` fields can drive the `ApprovalRequired` / `ToolDenied` / `ToolFailed`
mapping documented on `AlgentaToolset.call_tool`.
...
`GovernedExecutionReceipt`, and `AlgentaToolset.call_tool` passes such results through unchanged
as an ordinary successful tool result.
"""
...
- `"pending"`: the call is paused awaiting an out-of-band policy approval -- see
  `AlgentaToolset.call_tool`, which raises
  [`ApprovalRequired`][pydantic_ai.exceptions.ApprovalRequired] for this state rather than
  returning it as an ordinary result.
...
#: `AlgentaToolset.call_tool` maps it to `ToolReturnPart(outcome="denied")`, matching
#: `approval_state == "rejected"`, rather than `outcome="failed"`.
...
#: Consulted only once `approval_state` and `code` have already been checked -- see
#: `AlgentaToolset.call_tool`.
...
def parse_receipt(
    ...
    """Parse a wrapped MCP tool's raw result into a `GovernedExecutionReceipt`, or `None`.
    ...
    This is the deliberate signal `AlgentaToolset.call_tool` uses to pass a non-governed tool's
    result (such as `get_contract`'s discovery payload) through unchanged.

    Args:
        raw_result: The wrapped MCP tool's raw result (typically a dict).
        model: The `GovernedExecutionReceipt` subclass to validate against -- pass
