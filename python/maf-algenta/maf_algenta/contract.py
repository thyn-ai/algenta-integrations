...
`pydantic_ai_algenta.contract`, `langchain_algenta.contract`, and `typescript/algenta-tools`'s
`src/contract.ts` -- the tool-profile boundary is one shared decision, not something each
framework package gets to redefine.
"""
...
#: layers: it strips these keys from a tool's advertised JSON schema (the `input_model` handed
#: to `agent_framework.FunctionTool`) *and* from the arguments dict actually forwarded to the
#: wrapped MCP tool call, so a model that somehow still emitted one of these as an argument (e.g.
#: by copying it from a prior message, or because MAF's own lightweight argument validator does
#: not reject unknown properties -- see `maf_algenta.toolset`'s module docstring) can't get it
