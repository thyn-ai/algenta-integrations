...
`pydantic_ai_algenta.contract`, `langchain_algenta.contract`, `maf_algenta.contract`,
`haystack_algenta.contract`, and `typescript/algenta-tools`'s `src/contract.ts` -- the tool-profile
boundary is one shared decision, not something each framework package gets to redefine.
"""
...
#: discovery/capability listing, never a governed-execution receipt). Used only as a
#: documentation aid; `llamaindex_algenta.receipts.parse_receipt` detects a governed-execution
#: receipt by *shape*, not by tool name -- exactly matching the contract's own "a result missing
#: status/code should pass through unchanged" wording.
...
#: contract's `profiles.execute.never_model_facing_note`. This package enforces that at two
#: layers: it strips these keys from a tool's advertised JSON schema (the generated pydantic
#: `fn_schema`) *and* from the arguments dict actually forwarded to the wrapped MCP `call_tool`,
#: so a model that somehow still emitted one of these as an argument (e.g. by copying it from a
#: prior message) can't get it through either layer.
