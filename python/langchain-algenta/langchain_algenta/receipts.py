...
as its result. `langchain_algenta.interceptor.AlgentaToolCallInterceptor` parses the wrapped MCP
tool's raw result into this model so the approval/denial/failure mapping (see that module) can
be driven off typed fields (`approval_state`, `code`, `status`) instead of raw dict indexing.
...
and `typescript/algenta-tools`'s `src/receipts.ts` -- the receipt envelope is one shared contract,
...
- `"pending"`: the call is paused awaiting an out-of-band policy approval -- see
  `langchain_algenta.interceptor.AlgentaToolCallInterceptor`, which pauses the enclosing
  LangGraph run via `langgraph.types.interrupt(...)` for this state rather than returning it as
  an ordinary result.
...
#: `AlgentaToolCallInterceptor` raises `AlgentaToolDenied` for this, matching
...
def parse_receipt(
    ...
    """Parse a wrapped MCP tool's raw result payload into a `GovernedExecutionReceipt`, or `None`.
    ...
    Args:
        raw_result: The wrapped MCP tool's raw result payload, already unwrapped from the MCP
            `CallToolResult` envelope (typically a dict -- see
