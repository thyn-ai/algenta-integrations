...
that layers profile enforcement, never-model-facing scrubbing, and governed-execution receipt
handling onto every real MCP tool call made through a `langchain_mcp_adapters.MultiServerMCPClient`.

Structural template: `langchain_mcp_adapters.interceptors.ToolCallInterceptor`, the adapter
library's own native seam for "wrap every tool call this client's tools make, including
short-circuiting or retrying the real network call" -- deliberately *not* a manual re-wrap of
each `StructuredTool` `get_tools()` returns (see the package README's "Why an interceptor, not a
re-wrapped `StructuredTool`" section, and `langchain_algenta.toolset.create_algenta_tools`'s
docstring for the one place this package still does the latter, for tools that never went
through an MCP client at all).
"""
...
def extract_call_tool_payload(call_result: CallToolResult) -> Any:
    """Unwrap the actual JSON payload out of a raw MCP `CallToolResult` envelope.
    ...
    produce), and otherwise returns `None`. Mirrors
    `typescript/algenta-tools`'s `extractToolPayload` in `src/toolset.ts`.

    This function only ever *inspects* the result to decide whether it's a governed-execution
    receipt -- it never replaces what actually gets returned to the framework. See
    `AlgentaToolCallInterceptor.__call__`, which always returns the original `CallToolResult`
    (or a fresh one from a retry) unchanged on the passthrough/success path, so
    `langchain_mcp_adapters`' own `isError`/`structuredContent` handling still runs downstream
    exactly as it would without this package involved at all.
    """
...
class AlgentaToolCallInterceptor:
    """Enforces tool-profile membership, never-model-facing scrubbing, and the governed-execution
    approval/denial/failure mapping on every call `handler` makes.
    ...
    async def __call__(
        ...
        3. **Governed-execution mapping**: calls `handler(request)`, extracts the payload from
           the resulting `CallToolResult`, and resolves it via
           `langchain_algenta.governance.resolve_governed_call` -- which may call `handler` a
           second time (this protocol's own documented, intended use of a retriable `handler`)
           after a `langgraph.types.interrupt(...)` pause, for a `"pending"` approval.

        On the passthrough/success path, returns the raw `CallToolResult` completely unchanged
        (not the parsed receipt) -- see `resolve_governed_call`'s docstring for why.
        """
        ...
        async def _call() -> tuple[CallToolResult, Any]:
            raw_result = await handler(request)
            if not isinstance(raw_result, CallToolResult):
                # A prior interceptor in the chain already short-circuited to a ToolMessage/
                # Command -- nothing left for this interceptor to parse as a receipt.
                return raw_result, None
            return raw_result, extract_call_tool_payload(raw_result)

        raw_result, payload = await _call()
        return await resolve_governed_call(
            request.name, raw_result, payload, receipt_model=self.receipt_model, retry=_call
        )
