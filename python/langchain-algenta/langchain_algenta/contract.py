...
This module is deliberately identical in shape (constants, names, docstrings) to its sibling
in `pydantic_ai_algenta.contract` and to `typescript/algenta-tools`'s `src/contract.ts` -- the
tool-profile boundary is one shared decision, not something each framework package gets to
redefine.
"""
...
#: layers: it strips these keys from a tool's advertised JSON schema (the list of tools handed
#: to the model) *and* from the arguments dict actually forwarded to the wrapped MCP tool call,
#: so a model that somehow still emitted one of these as an argument (e.g. by copying it from a
#: prior message) can't get it through either. See `langchain_algenta.interceptor` and
#: `langchain_algenta.toolset`.
...
def is_tool_allowed_for_profile(name: str, profile: ToolProfile) -> bool:
    ...
    Used as the interceptor's call-time, defense-in-depth check (see
    `langchain_algenta.interceptor.AlgentaToolCallInterceptor`) -- unlike
