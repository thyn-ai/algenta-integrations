...
real `mcp` client -> real wire -> this server round trip, so a wire-shape regression (e.g. the
receipt envelope not surviving structured-content round-tripping) would actually be caught here,
unlike a test that mocks tool execution directly. Mirrors the design of
`python/pydantic-ai-algenta/tests/stub_server.py` (same plan-hash fixtures, same scenarios),
adapted to this package's transport library.
...
envelope (see `langchain_algenta.receipts.GovernedExecutionReceipt`), plus one test-only
